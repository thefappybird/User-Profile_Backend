...
describe("Auth Controllers", () => {
  ...
  describe("register", () => {
    ...
    it("should create user and log action", async () => {
      ...
      (hashPassword as jest.Mock).mockResolvedValue("hashedpw");
      (User.create as jest.Mock).mockResolvedValue({
        id: "1",
        name: "Alice",
        email: "a@b.com",
      });
      (Log.create as jest.Mock).mockResolvedValue({});
      ...
      expect(User.create).toHaveBeenCalledWith(
        { name: "Alice", email: "a@b.com", password: "hashedpw" },
        { transaction: mockTransaction }
      );
      expect(Log.create).toHaveBeenCalledWith(
        { user_id: "1", action: "Register User" },
        { transaction: mockTransaction }
      );
